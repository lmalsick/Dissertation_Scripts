...
sfrna12 <- read_csv("~/Lauren_PhD/Geiss_Lab_PhD_Project/Bat_cell_project/Genewiz_sequencing/03_output/Virus_timepoint_to_mock_logfoldchange_0.5/Gene_Lists/virus_timepoint_853.72_vs_Mock.72_allGenes.csv")
...
# ---- Merge datasets ----
merged <- inner_join(sfrna12, west, by = "Gene", suffix = c("_sfrna12", "_west")) %>%
  filter(!is.na(log2FoldChange_sfrna12), !is.na(log2FoldChange_west))

merged_clean <- merged %>%
  filter(is.finite(log2FoldChange_sfrna12), is.finite(log2FoldChange_west))

# Annotate significance status
merged_clean <- merged_clean %>%
  mutate(sig_status = case_when(
    padj_sfrna12 < 0.005 & padj_west < 0.005 ~ "Both significant",
    padj_sfrna12 < 0.005 ~ "sfRNA1and2 Mutant only",
...
differential_genes <- merged_clean %>%
  mutate(
    ...
    mutant_near_zero = abs(log2FoldChange_sfrna12) < 1.5,  # Mutant close to 0
    wt_strong = abs(log2FoldChange_west) > 3,  # WT strongly regulated
    mutant_strong = abs(log2FoldChange_sfrna12) > 3,  # Mutant strongly regulated
    wt_significant = padj_west < 0.05,  # WT significant
    mutant_significant = padj_sfrna12 < 0.05,  # Mutant significant
...
top_mutant_specific <- differential_genes %>%
  ...
  filter(padj_sfrna12 < 0.005 | padj_west < 0.005) %>%
  arrange(desc(abs(log2FoldChange_sfrna12))) %>%
  ...
  color_check <- highlight_genes %>%
    mutate(
      will_be_colored = case_when(
        padj_sfrna12 < 0.005 & padj_west < 0.005 ~ "Both significant (RED)",
        padj_sfrna12 < 0.005 ~ "sfRNA1and2 Mutant only (BLUE)",
...
colors_enhanced <- c(
  ...
  "sfRNA1and2 Mutant only" = "#1F78B4",   # Blue  
  "WNV only" = "#33A02C",             # Green
  "Not significant" = "grey75"         # Light grey
)

########################### ---- PLOT 1:  ----
p1_enhanced <- ggplot(merged_clean, aes(x = log2FoldChange_west, y = log2FoldChange_sfrna12)) +
  ...
  labs(
    ...
    subtitle = "WT WNV vs sfRNA1and2 Mutant",
    x = "log2 Fold Change (WT WNV)",
    y = "log2 Fold Change (WNV sfRNA1and2 Mutant)"
  ) +
...
write_csv(merged_clean, "WNVandsfrna12_correlation_plot_data.csv")

print(p1_enhanced)

################################### ---- PLOT 2:  ----
diagonal_crossers <- merged_clean %>%
  filter(sign(log2FoldChange_sfrna12) != sign(log2FoldChange_west)) %>%
  filter(padj_sfrna12 < 0.05 | padj_west < 0.05) %>%
  mutate(cross_direction = case_when(
    log2FoldChange_sfrna12 > 0.1 & log2FoldChange_west < 0.1 ~ "Up in sfRNA1and2, Down in WNV",
    log2FoldChange_sfrna12 < 0.1 & log2FoldChange_west > 0.1 ~ "Down in sfRNA1and2, Up in WNV"
  )) %>%
...
n_labels <- 5
top_sfrna12 <- diagonal_crossers %>%
  top_n(n_labels, wt = abs(log2FoldChange_sfrna12))
top_west <- diagonal_crossers %>%
  top_n(n_labels, wt = abs(log2FoldChange_west))
label_genes <- unique(c(top_sfrna12$Gene, top_west$Gene))

p2_enhanced <- ggplot(diagonal_crossers, aes(x = log2FoldChange_west, y = log2FoldChange_sfrna12)) +
  ...
  scale_color_manual(values = c(
    "Up in sfRNA1and2, Down in WNV" = "#1F78B4",
    "Down in sfRNA1and2, Up in WNV" = "#33A02C"
  ...
  labs(
    ...
    subtitle = "Divergent Expression Patterns Between sfRNA1and2 Mutant and WT",
    x = "log2 Fold Change (WT WNV)",
    y = "log2 Fold Change (WNV sfRNA1and2 Mutant)"
  ) +
  ...
  highlight_summary <- highlight_genes %>%
    mutate(
      regulation_type = case_when(
        abs(log2FoldChange_west) < 1.5 & abs(log2FoldChange_sfrna12) > 3 ~ "Mutant-specific",
        abs(log2FoldChange_sfrna12) < 1.5 & abs(log2FoldChange_west) > 3 ~ "WT-specific",
        TRUE ~ "Other"
      )
    ) %>%
    select(Gene, log2FoldChange_west, log2FoldChange_sfrna12, padj_west, padj_sfrna12, regulation_type) %>%
    arrange(regulation_type, desc(abs(log2FoldChange_sfrna12) + abs(log2FoldChange_west)))
